    ...
    <div className="h-screen flex flex-col bg-[#0d1117] overflow-hidden">
      ...
      <div className="border-b border-[#21262d] bg-[#0d1117] px-5 py-2 shrink-0">
        <div className="flex items-center gap-2.5 overflow-x-auto">
          ...
          {totalFormLetters > 0 && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-md border border-[#f85149]/20 bg-[#f85149]/5 shrink-0">
              <span className="h-1.5 w-1.5 rounded-full bg-[#f85149] animate-pulse-glow" />
              <span className="text-[10px] tracking-wider uppercase text-[#f85149] font-semibold whitespace-nowrap" style={mono}>
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden">
        {/* PANEL 1: Live Stream */}
        <div className="w-[300px] shrink-0 border-r border-[#21262d] flex flex-col">
          <PanelHead title="Live Stream" sub="Agent 1 / Haiku 4.5" badge={`${processed}/${total}`} icon={<Activity className="h-3.5 w-3.5 text-[#58a6ff]" />} />
          <div ref={streamRef} className="flex-1 overflow-y-auto p-2 space-y-1.5">
            {/* Campaign banners */}
            {Object.values(campaigns).map((camp) => (
              <div key={camp.id} className="rounded border border-[#f85149]/20 bg-[#f85149]/5 px-2.5 py-2 animate-card-enter">
                <div className="flex items-center justify-between">
                  <span className="text-[9px] tracking-wider uppercase font-bold text-[#f85149]" style={mono}>{camp.id.replace(/_/g, " ")}</span>
                  <span className="text-[10px] font-bold text-[#f85149] tabular-nums" style={mono}>{camp.count.toLocaleString()}x</span>
                </div>
                <div className="mt-1 text-[10px] text-[#8b949e] truncate italic">&quot;{camp.sampleText}&quot;</div>
              </div>
            ))}
                ...
                {clusters
                  ...
                  .map((cluster) => {
                    ...
                    return (
                      ...
                      >
                        <button className="w-full text-left px-4 py-3" onClick={() => toggleClusterExpand(cluster.cluster_id)}>
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="text-[9px] tracking-wider uppercase font-bold text-[#0d1117] bg-[#8b949e] px-1.5 py-0.5 rounded" style={mono}>
                                {cluster.policy_family}
                              </span>
                              {cluster.requires_response && (
                                <span className="text-[9px] tracking-wider uppercase font-bold text-[#3fb950] bg-[#3fb950]/10 border border-[#3fb950]/30 px-1.5 py-0.5 rounded">
                                  must respond
                                </span>
                              )}
                              {cluster.expert_count > 0 && (
                                <span className="text-[9px] tracking-wider uppercase font-bold text-[#f0a500] bg-[#f0a500]/10 border border-[#f0a500]/30 px-1.5 py-0.5 rounded">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              <span className="text-[10px] text-[#8b949e] tabular-nums" style={mono}>{cluster.comment_ids.length}</span>
                              {isExpanded ? <ChevronDown className="h-3.5 w-3.5 text-[#484f58]" /> : <ChevronRight className="h-3.5 w-3.5 text-[#484f58]" />}
                            </div>
                          </div>
                          <div className="mt-2 text-sm font-semibold text-[#e6edf3] leading-snug" style={display}>
                            {cluster.representative_claim}
                          </div>
                        </button>

                        {isExpanded && (
                          <div className="px-4 pb-3 border-t border-[#21262d]">
                            <p className="mt-3 text-xs text-[#8b949e] leading-relaxed">{cluster.argument_summary}</p>
                            {lowCohesion && (
                              <div className="mt-2 flex items-center gap-1.5 text-[10px] text-[#f0a500]">
                            ...
                            <button
                              ...
                              className="mt-3 flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-[#f0a500] text-[#0d1117] text-xs font-bold hover:bg-[#ffb519] transition-colors"
                            >
        ...
        <div className="w-[420px] shrink-0 flex flex-col">
          <PanelHead title="Response Document" sub="Draft Staff Review / ORS 183.335" icon={<FileText className="h-3.5 w-3.5 text-[#bc8cff]" />} />
          <div className="flex-1 overflow-y-auto">
            {!selectedCluster && !responding && (
              <div className="h-full flex items-center justify-center px-8">
                <div className="text-center">
                  <Scale className="h-10 w-10 text-[#30363d] mx-auto" />
                  <p className="mt-4 text-sm text-[#484f58] leading-relaxed">
                    Select a policy argument family, expand it, and click &quot;Generate Draft Response&quot; to produce the staff review scaffold.
                  </p>
                  <p className="mt-2 text-[10px] text-[#484f58]" style={mono}>
    ...
    <div className="m-3 rounded-md overflow-hidden">
      <div className="bg-[#faf8f5] p-6 gov-doc">
        ...
        <div className="border-b-2 border-[#1a1a1a] pb-3 mb-5">
          <div className="text-[9px] tracking-[0.25em] uppercase text-[#8b0000] font-bold" style={mono}>
            Draft Staff Review Response
          </div>
          <div className="text-[9px] tracking-[0.15em] uppercase text-[#666] mt-1" style={mono}>
            OR-DEQ-2026-AQ-014 / Proposed Amendments to OAR 340-200
          </div>
          <div className="text-[9px] tracking-[0.15em] uppercase text-[#8b0000] mt-0.5 font-semibold" style={mono}>
        ...
        {/* Policy family */}
        <div className="mb-5">
          <div className="text-[10px] tracking-[0.15em] uppercase text-[#666] font-semibold" style={mono}>
            Policy Family: {response.policy_family}
          </div>
          <h3 className="text-base font-bold text-[#1a1a1a] mt-1.5 leading-snug" style={{ fontFamily: "Georgia, 'Times New Roman', serif" }}>
            {cluster.representative_claim}
          </h3>
        </div>
        ...
        <div className="space-y-3">
          {paragraphs.map((para, i) => (
            <p key={i} className="text-[13px] leading-[1.75] text-[#2a2a2a]">
        ...
        <div className="mt-6 border-t border-[#ccc] pt-4">
          <div className="text-[10px] tracking-[0.15em] uppercase text-[#666] font-bold mb-2" style={mono}>
            Source Comments
          </div>
          <div className="space-y-1.5">
            {response.citations.map((cite) => (
              <button
                ...
                className="w-full text-left rounded border border-[#ddd] bg-white px-3 py-2 hover:bg-[#fff8e7] hover:border-[#f0a500] transition text-xs"
                style={{ fontFamily: "system-ui, sans-serif" }}
              >
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-[#b37800] font-bold underline" style={mono}>{cite.comment_id}</span>
                  <span className="text-[#333] font-medium">{cite.submitter}</span>
                  {cite.submitter_org && <span className="text-[10px] text-[#666]">({cite.submitter_org})</span>}
                </div>
                <div className="mt-1 text-[11px] text-[#555] italic line-clamp-2">&quot;{cite.quote}&quot;</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="mt-6 border-t-2 border-[#1a1a1a] pt-3">
          <p className="text-[10px] text-[#888] italic leading-relaxed" style={{ fontFamily: "system-ui, sans-serif" }}>
            This document was generated by Cascade as structured intelligence for staff review. It does not constitute an
            official agency determination and requires human analyst review before any official action. All citations
            reference comments submitted to docket OR-DEQ-2026-AQ-014. Bidirectional traceability is maintained for
            judicial review compliance under APA Section 553.
          </p>
        </div>
      </div>
    </div>
...
function Label({ children }: { children: React.ReactNode }) {
  return <div className="text-[9px] tracking-[0.2em] uppercase text-[#8b949e] font-bold" style={mono}>{children}</div>;
}

function ArgField({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <span className="text-[10px] tracking-wider uppercase text-[#f0a500] font-bold" style={mono}>{label}: </span>
      <span className="text-xs text-[#e6edf3]">{value}</span>
    </div>
  );
}
...
function CatBadge({ category }: { category: string }) {
  ...
  return (
    <span className={`text-[9px] tracking-wider uppercase font-bold border rounded px-1.5 py-0.5 ${c[category] || ""}`} style={mono}>
...
function StreamCard({ comment }: { comment: ClassifiedComment }) {
  ...
  return (
    <div className={`rounded border-l-2 ${CAT_BORDERS[comment.category] || "border-l-[#30363d]"} border border-[#21262d] bg-[#161b22] px-2.5 py-2 text-xs animate-card-enter ${isExpert ? "ring-1 ring-[#f0a500]/20" : ""}`}>
      <div className="flex items-center justify-between">
        <span className="text-[10px] text-[#484f58]" style={mono}>{comment.id}</span>
        <span className={`text-[9px] tracking-wider uppercase font-bold ${CAT_COLORS[comment.category] || "text-[#8b949e]"}`} style={mono}>
          {CAT_LABELS[comment.category]}
        </span>
      </div>
      <div className="mt-1 font-medium text-[#e6edf3] truncate">
        {comment.submitter}
        {comment.submitter_org && <span className="text-[#8b949e] font-normal"> ({comment.submitter_org})</span>}
      </div>
      <div className="mt-1 text-[#8b949e] line-clamp-2">{comment.text.slice(0, 140)}</div>
      <div className="mt-1 text-[10px] text-[#484f58]">{(comment.confidence * 100).toFixed(0)}%  &middot;  {comment.reasoning}</div>
    </div>
  );
}
...
function PhaseChip({ phase }: { phase: Phase }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-semibold tracking-wider uppercase border ${cls}`} style={mono}>
...
function Metric({ label, value, color, sub, highlight, active }: { label: string; value: string; color: string; sub?: string; highlight?: boolean; active?: boolean }) {
  return (
    <div className={`flex items-center gap-2.5 px-3 py-1.5 rounded-md border ${highlight ? "border-[#f0a500]/40 bg-[#f0a500]/5" : "border-[#21262d] bg-[#161b22]"}`}>
      <div className="h-1.5 w-1.5 rounded-full shrink-0" style={{ backgroundColor: color }} />
      <div>
        <div className="text-[9px] tracking-wider uppercase text-[#484f58] font-semibold leading-none" style={mono}>{label}</div>
        <div className="flex items-center gap-1.5 mt-0.5">
          <span className="text-sm font-bold tabular-nums leading-none" style={{ color, ...mono }}>{value}</span>
          {sub && <span className="text-[10px] text-[#484f58]">{sub}</span>}
          {active && <Loader2 className="h-3 w-3 animate-spin" style={{ color }} />}
        </div>
      </div>
    </div>
  );
...
function PanelHead({ title, sub, badge, icon }: { title: string; sub: string; badge?: string; icon: React.ReactNode }) {
  return (
    <div className="border-b border-[#21262d] px-4 py-2.5 flex items-center justify-between bg-[#161b22] shrink-0">
      <div className="flex items-center gap-2">
        {icon}
        <div>
          <div className="text-xs font-semibold text-[#e6edf3]">{title}</div>
          <div className="text-[9px] text-[#484f58] tracking-wider uppercase" style={mono}>{sub}</div>
        </div>
      </div>
      {badge && <span className="text-[10px] text-[#8b949e] tabular-nums" style={mono}>{badge}</span>}
    </div>
  );
}
